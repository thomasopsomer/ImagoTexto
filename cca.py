...
def cca(word_emb, image_emb, d=128):
    ...
    S_i_w = np.matmul(image_emb.T, word_emb)
    # assemble whole covariance matrix A
    A = np.bmat([[S_w_w, S_w_i], [S_i_w, S_i_i]])
    ...
    # eigvenvalues, eigenvectors = scipy.linalg.eigh(A,B)
...
if __name__ == '__main__':
    # test
    word_emb = np.random.random_sample(size=(10000, 128))
    image_emb = np.random.random_sample(size=(10000, 4096))
    # perform cca
